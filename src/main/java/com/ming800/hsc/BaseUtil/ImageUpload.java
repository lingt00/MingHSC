...
public class ImageUpload {
    ...
    @RequestMapping(value = "/uploadify.do",method = RequestMethod.POST)
        ...
        if (mutilpartResolver.isMultipart(request)) {
            ...
            if (it.hasNext()) {
                ...
                if (fileDetail != null) {
                    ...
                    //   图片上传路径
                    String uploadPath =request.getSession().getServletContext().getRealPath("/")+"upload/images/"+date+"/";
                    //   图片网络相对路径
//                    String imagePath=request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/upload/images/"+date+"/";
                    //上传记录相对路径
                    String imagePath=request.getContextPath()+"/upload/images/"+date+"/";
    ...
    @RequestMapping(value = "/uploadImgLrs.do",method = RequestMethod.POST)
    public void uploadImgJson(HttpServletRequest request,HttpServletResponse response) throws Exception {
        response.setContentType("text/plain");
        request.setCharacterEncoding("utf-8");
        ...
        //   图片上传路径
        String uploadPath =request.getSession().getServletContext().getRealPath("/")+"upload"+File.separator+"images"+File.separator+date+File.separator;
        //   图片网络相对路径
        String imagePath=request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/upload/images/"+date+"/";
        //如果此文件夹不存在则创建一个
        //   文件夹不存在就自动创建
        ...
        File findNameFile=null;//查找重名对象
        do{
            newName=destinationfileName+"-"+PinYinUtil.parsePinYin(name)+"-"+String.valueOf(rd.nextInt(10000))+suffix;
            findNameFile=new File(uploadPath+newName);
        }while(findNameFile.exists());
